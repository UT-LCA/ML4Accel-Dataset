...
def run_routine(prj_list, prj_dir, kernel_name, csv_file):
    ...
    for prj_name in prj_list:
        ...
        RSC_file = '{}/{}/solution/.autopilot/db/{}.verbose.rpt.xml'.format(prj_dir, prj_name, kernel_name)
        ...
        if os.path.exists(RSC_file):
            ...
            #remove bad designs
            if temp_dict['bram'] > 1824 or temp_dict['dsp'] > 2520 or temp_dict['ff'] > 548160 or temp_dict['lut'] > 274080:
                continue

            global_info_dic[prj_name] = [temp_dict['clk_target'], 
    ...
    with open(file_path, 'w+', newline='') as f:
        title = ['prj','clk_target(ns)', 'clk_estimated(ns)', 'clk_uncertainty(ns)', 'bram', 'dsp', 'ff', 'lut', 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_file', required=True, help = "output: csv file containing info")
    parser.add_argument('--kernel_name', required=True, help = "input:top kernel_name of HLS function")
    ...
    print('Finished feature extraction: project num = {} for {}'.format(prj_cnt, kernel_name))
