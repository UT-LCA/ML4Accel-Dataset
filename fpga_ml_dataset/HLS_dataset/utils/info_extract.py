...
def gen_script(fw, prj_dir, prj_name):
    ...
    fw.write(
        "report_utilization -hierarchical -file {}/{}/utilization.xls\n".format(
            prj_dir, prj_name
        )
    )
    fw.write(
        "report_timing -file {}/{}/slack.xls -sort_by slack\n".format(prj_dir, prj_name)
    )
    fw.write(
        "report_timing_summary -setup -hold -file {}/{}/timing.xls\n".format(
            prj_dir, prj_name
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="automatic Vivado bitstream & report extraction", epilog=""
    )
    parser.add_argument(
        "--script_num",
        help="number of scripts to run parallely",
        action="store",
        default=1,
    )
    parser.add_argument(
        "--prj_dir", help="directory of the vivado projects", action="store"
    )
