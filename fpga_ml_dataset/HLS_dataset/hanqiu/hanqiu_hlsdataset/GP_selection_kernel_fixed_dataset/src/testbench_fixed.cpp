...
void readFloatsFromFile_2d(const char *filename, float numbers[N_CLIENT][N_CLIENT], int rows, int cols) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        perror("Error opening file");
        exit(EXIT_FAILURE);
...
extern void GP_kernel_fixed(DATA_TYPE *mu_dram,  DATA_TYPE sigma_dram[N][N], DATA_TYPE *a_dram, DATA_TYPE *beta_dram, int client_number_array[10], int client_index_list[C], DATA_TYPE result_dram[N][N], DATA_TYPE result_dram_2[N]);
...

{
    ...
    int client_num = 6; //Change here between 1-6 for the fixed point version
    ...
    //     {
    //         sigma_float[i][j] = (float)rand() / RAND_MAX + 0.5;
    //     }
