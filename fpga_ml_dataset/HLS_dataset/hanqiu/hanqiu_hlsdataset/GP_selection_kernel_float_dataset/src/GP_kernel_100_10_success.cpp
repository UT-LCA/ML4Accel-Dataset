...
#pragma HLS unroll
        ...
        delta_l_buffer[client_index] = mu_buffer[client_index] - delta_l_buffer[client_index] * sqrt(abs(sigma_buffer[client_index]));
...
void GP_kernel_float(DATA_TYPE *mu_dram,  DATA_TYPE sigma_dram[N][N], DATA_TYPE *a_dram, DATA_TYPE *beta_dram, int client_number_array[10], int client_index_list[C], DATA_TYPE result_dram[N][N], DATA_TYPE result_dram_2[N])
