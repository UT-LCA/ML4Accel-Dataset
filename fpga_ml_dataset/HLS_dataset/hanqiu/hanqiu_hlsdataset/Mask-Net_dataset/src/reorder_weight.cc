...
//define the new branch
FIX_WT fix_new_branch_conv_5x5_weight[32][6][5][5];
FIX_WT fix_new_branch_conv_5x5_bias[32];
FIX_WT fix_new_branch_conv_3x3_weight[32][3][3];
...
//define the new branch
extern uint512 fix_conv_weight_5x5_all_512bit[6][5][5];
...
void reorder_weight_fix()
{
	...
	//for new branch conv5x5

	for (int m = 0; m < 5; m++) {
		for (int n = 0; n < 5; n++) {
			for (int co = 0; co < 32; co++) {
				
					fix_new_branch_conv_5x5_bias[co] = new_branch_conv_5x5_bias[co];
					for (int ci = 0; ci < 6; ci++) {
						fix_new_branch_conv_5x5_weight[co][ci][m][n] = (FIX_WT)new_branch_conv_5x5_weight[co][ci][m][n];
	...
	// for new branch conv3x3

	for (int m = 0; m < 3; m++) {
		for (int n = 0; n < 3; n++) {
			for (int c = 0 ; c < 32 ; c++){
				
					fix_new_branch_conv_3x3_weight[c][m][n] = (FIX_WT)new_branch_conv_3x3_weight[0][c][m][n];
	...
	// reorder new branch conv5x5 weights and bias
	...
	for (int ci = 0; ci < 6; ci++) {
		...
		for (int co = 0; co < 32; co++) {
				...
				for (int m = 0; m < 5; m++) {
					for (int n = 0; n < 5; n++) {
						fix_conv_weight_5x5_all[co][index_5x5][m][n] = fix_new_branch_conv_5x5_weight[co][ci][m][n];
					}
				}
	...
	for (int co = 0; co < 32; co++) {
		index_bias = 106;
		fix_bias_all[index_bias][co] = fix_new_branch_conv_5x5_bias[co];
	}
	

	// reorder new branch conv3x3 weights

	for (int c = 0; c < 32; c++) {
		index_3x3 = 64;
		for (int m = 0; m < 3; m++) {
			for (int n = 0; n < 3; n++) {
				fix_conv_weight_3x3_all[index_3x3][c][m][n] = fix_new_branch_conv_3x3_weight[c][m][n];
			}
		}
	}
	...
	for (int ci = 0; ci < 6; ci++) {
		for (int co = 0; co < 32; co++) {
				...
				for (int m = 0; m < 5; m++) {
					for (int n = 0; n < 5; n++) {
						uint16 DATA = 0;
						DATA.range(10, 0) = fix_conv_weight_5x5_all[co][ci][m][n].range(10, 0);
						fix_conv_weight_5x5_all_16[co][ci][m][n].range(15, 0) = DATA.range(15, 0);
					}
				}
...
//	ofs_param_write_16.close();
	...
	//std::ofstream ofs_param_write_512("C://Users//chenhanqiou//Desktop//SkyNet-master-hlsnet//SkyNet-master//FPGA//HLS_version-14//weights_fixed_finetune.bin", std::ios::out | std::ios::binary);
	std::ofstream ofs_param_write_512("weights_fixed_DAC.bin", std::ios::out | std::ios::binary);
	...
	for (int c = 0; c < index_5x5; c++) {
		
			for (int m = 0; m < 5; m++) {
				for (int n = 0; n < 5; n++) {
					uint512 DATA = 0;
					for (int j = 0; j < 32; j++) {
						DATA.range(j * 16 + WT_RG, j * 16) = fix_conv_weight_5x5_all[j][c][m][n].range(WT_RG, 0);
					}
					fix_conv_weight_5x5_all_512bit[c][m][n].range(511, 0) = DATA.range(511, 0);
				}
			}
	...
	ofs_param_write_512.write((char*)fix_conv_weight_5x5_all_512bit, index_5x5 * 5 * 5 * sizeof(uint512));
